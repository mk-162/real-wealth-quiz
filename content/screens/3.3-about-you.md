---
id: screen.3.3.about_you
screen_number: "3.3"
title: About you
section: life_shape
layout: asymmetric
grouped: true
gate_critical: true
tier_limit: [A, B, C]
image_family: family_1_life_shape
image_direction: "A domestic scene, a Sunday newspaper on a breakfast table, two place settings. Warm, unpeopled."
q_refs: ["Q2.1", "Q2.2", "Q2.2a", "Q2.2b"]
audience:
  "Q2.1":
    S1: shown
    S2: shown
    S3: shown
    S4: shown
    S5: shown
    S6: shown
    S7: shown
    S8: shown
    S9: shown
  "Q2.2":
    S1: shown
    S2: shown
    S3: shown
    S4: shown
    S5: shown
    S6: shown
    S7: shown
    S8: shown
    S9: shown
  "Q2.2b":
    S1: shown
    S2: shown
    S3: shown
    S4: shown
    S5: shown
    S6: shown
    S7: shown
    S8: shown
    S9: shown
logged_as: [age, household, your_gross_income, youngest_child_band, scottish_taxpayer]
inputs:
  - id: age
    label: "Your age"
    control: slider
    range: { min: 25, max: 80, default: 45, step: 1 }
    required: true
  - id: scottish_taxpayer
    label: "Where do you pay income tax?"
    label_helper: "Pick the country where you spend most of the tax year."
    control: radio
    required: true
    options:
      - value: 'no'
        label: "England, Wales, or Northern Ireland"
      - value: 'yes'
        label: "Scotland"
  - id: your_gross_income
    label: "Your gross annual income (£)"
    label_helper: "Salary and dividends, before tax. Leave at 0 if you're not currently working."
    control: slider
    required: true
    range: { min: 0, max: 300000, default: 0, step: 5000 }
  - id: household
    label: "Who else is part of the plan?"
    control: multi_select
    required: true
    options:
      - value: partner
        label: "A partner"
        icon: heart
        conditional_reveal: "partner_age"
      - value: dependent_children
        label: "Dependent children"
        icon: baby
        conditional_reveal: "youngest_child_band"
      - value: adult_children
        label: "Adult children"
        icon: users
      - value: elderly_parent
        label: "An elderly parent or relative I support"
        icon: heart-hand
      - value: solo
        label: "Nobody else — just me"
        icon: user
      - value: complicated
        label: "It's complicated — I'll explain"
        icon: more-horizontal
  - id: youngest_child_band
    label: "Youngest child's age"
    control: radio
    required: false
    conditional_reveal: "only when household includes dependent_children"
    options:
      - value: under_5
        label: "Under 5"
      - value: 5_to_11
        label: "5–11"
      - value: 12_to_17
        label: "12–17"
  - id: partner_age
    label: "Your partner's age"
    control: slider
    required: false
    conditional_reveal: "only when household includes partner"
    range: { min: 25, max: 90, default: 45, step: 1 }
  - id: partner_gross_income
    label: "Their gross annual income (£)"
    label_helper: "Salary and dividends, before tax. Leave at 0 if they're not working."
    control: slider
    required: false
    conditional_reveal: "only when household includes partner"
    range: { min: 0, max: 300000, default: 0, step: 5000 }
  - id: partner_pension_value
    label: "Their pension pot (£)"
    label_helper: "All their pensions combined. Leave at 0 to skip."
    control: slider
    required: false
    conditional_reveal: "only when household includes partner"
    range: { min: 0, max: 2000000, default: 0, step: 25000 }
---

# Headline
Tell us a little about you.

# Sub
A few quick questions — they shape everything that follows.
