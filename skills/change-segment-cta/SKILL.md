...
description: Edit the headline, body, cta label, cta helper text, or booking link on one of the 11 Real Wealth segment CTA files (9 segments S1–S9 plus 2 overlays). Use this skill whenever the user asks to update the retiree CTA, change the S6 headline, soften the HNW call-to-action, tweak the cta helper line, update a Calendly link, or otherwise modify the summary-page CTA for a specific segment or overlay. Triggers on phrasings like "update S8's cta", "change the retired CTA", "rewrite the urgency-this-week overlay", or just "the CTA for business-exit folks".
---
...
Edits the `# Headline`, `# Body`, `# Cta`, or `# Cta Helper` body sections — or the `button_link` frontmatter field — on a single `content/segments/S<n>-*.md` or `content/segments/overlay-*.md` file. Preserves frontmatter and every other section.
...
- Body: `# Headline`, `# Body`, `# Cta`, `# Cta Helper`.
...
2. **Which field.** Headline, body, cta, cta helper, or booking link. If the user named the text change without naming the field, infer:
   - One punchy line → `# Headline`.
   - 1–3 sentence paragraph → `# Body`.
   - Button verb phrase → `# Cta`.
   - Small grey line under the button → `# Cta Helper`.
...
4. **Respect character budgets** (from `admin_app/shared/character-budgets.ts`):
   ...
   - `segment.cta` — ideal 22, hard 32.
   - `segment.cta_helper` — ideal 40, hard 60.
...
- **Never delete a body section.** All four (`Headline`, `Body`, `Cta`, `Cta Helper`) must exist — the admin integrity check enforces it.
...
### Example 2 — change the cta label

**User:** "S8 cta should say 'Book a drawdown review' — current one is generic."
...
Validation: `segment.cta` 22 chars — at ideal. Clean.
...
- **Cta text is button-shaped.** Imperative verb phrases — "Book a call", "Start your plan". Not "Book a call now!!!" (exclamation banned) and not "You should book a call" (advice-verb banned per `voice-rules.md`).
...
- **Cta Helper text is not a tagline.** It's a small reassurance under the button — "A 30-minute call with a senior partner." It's not "The future of advice, delivered." Keep it literal.
