...
description: Rewrite the headline, body, or close of an existing Real Wealth provocation — the short compliance-gated callouts fired on specific answer patterns like the £2m IHT cliff or the £100k tax trap. Use this skill whenever the user asks to soften a provocation, tighten its body, change the close line, reword the hook, polish the prose, or fix tone on a compliance callout. Triggers on phrasings like "soften the £2m cliff provocation", "rewrite the retired-short body", "the close on the pension-pots-tease provocation feels flat", or just "the provocation about inheritance tax".
---
...
Updates one or more of the `# Headline`, `# Body`, or `# Close` body sections on a single `content/provocations/<slug>.md` file. Preserves all frontmatter — trigger, segments, compliance_status, signoff dates — every structural field stays put.
...
2. **Which section.** Headline, body, or close. If not specified, infer from the length/shape of the intended rewrite — short punchy line → headline; paragraph → body; wrap-up line → close.
...
5. **Respect character budgets** (`admin_app/shared/character-budgets.ts`):
   ...
   - `provocation.close` — ideal 30, hard 50.
...
- **All three body sections must exist.** `# Headline`, `# Body`, `# Close`. Don't delete any of them.
...
### Example 3 — close line rewrite

**User:** "The close on the pension-pots-tease provocation should feel like an invitation, not a warning."

**Target:** `content/provocations/pension-pots-tease.md`. Edit only `# Close`. Preserve the rest.
...
- **The close line does real emotional work.** Provocations land with a concrete hook (headline), an explanation (body), and a single-line invitation (close). Keep the close functional — "One of the conversations most families come to us for." not "Book a call today!"
