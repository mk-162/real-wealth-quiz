<!-- _AUDIT.md entry: 5.1 (bundled stem/aware/partial/unaware edits) -->
---
name: edit-awareness-check
description: Rewrite the stem, aware body, partial body, or unaware body on an existing Real Wealth awareness check. Use this skill whenever the user asks to update an awareness check's prompt, soften the "aware" variant, rewrite the unaware body, tweak the partial-awareness wording, or otherwise modify the copy on one of the `content/awareness-checks/*.md` files. Triggers on phrasings like "rewrite the unaware body on will-currency", "soften the RNRB taper stem", "update all three bodies on emergency-fund-sizing", or just "the awareness check about NI gaps".
---
...
Updates one or more of the four body sections — `# Stem`, `# Aware body`, `# Partial body`, `# Unaware body` — on a `content/awareness-checks/<slug>.md` file. This skill bundles all four body edits under one skill because in practice they're edited together: a voice pass on the stem usually prompts refinements to the three variants.

## Background — what an awareness check is

Awareness checks are the "did you know this?" prompts that fire after relevant questions. Each has a stem (the question-style prompt) and three body variants tuned to the user's self-declared awareness level. Schema (from `content/schema.ts`):
...
- `# Stem` — the question-style prompt.
- `# Aware body` — shown if the user self-declares aware.
- `# Partial body` — shown if partial.
- `# Unaware body` — shown if unaware.
...
2. **Which section(s).** Stem, aware, partial, unaware, or some combination.
...
4. **Character budgets** (from `admin_app/shared/character-budgets.ts`):
   - `awareness.stem` — ideal 120, hard 180.
   - `awareness.aware_body` — ideal 200, hard 380.
   - `awareness.partial_body` — ideal 250, hard 460.
   - `awareness.unaware_body` — ideal 280, hard 500.

5. **Voice — stem is a question, bodies are explanations.** The stem asks "were you aware of X?" — polite, direct, no leading. The three bodies tune depth:
   - **Aware** — confirms and offers the adviser angle. "You already know this; here's what we'd do about it."
   - **Partial** — fills gaps. "You have the outline; here's the numeric detail."
   - **Unaware** — explains fresh. "This matters because…; typical impact is…"
...
- **All four body sections must remain present.** `# Stem`, `# Aware body`, `# Partial body`, `# Unaware body`. None can be deleted.
...
### Example 3 — stem tweak

**User:** "The stem on the NI gaps check is a bit wordy."

Target: `content/awareness-checks/ni-gaps.md`. Edit `# Stem` only. Keep under ideal 120 chars.
...
- **Section headings matter exactly.** The loader reads `# Stem`, `# Aware body`, `# Partial body`, `# Unaware body`. Case-sensitive on the word, case-insensitive on the whitespace. Don't rename.
...
- **Stems are questions, not imperatives.** End with "were you aware of that?" or "did you know…" — not "Consider whether…" or "You should think about…"
