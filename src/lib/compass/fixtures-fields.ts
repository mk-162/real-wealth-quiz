...
const fieldsView: SegmentView = {
  ...
  healthInterpretation: '{view.healthInterpretation} — zone-matched copy from content/pdf-report/health-gauge.md, picked by score',
  headline: {
    tone: 'warn',
    title: '{view.headline.title} — from content/pdf-report/takeaway-banners.md / H1 segment / ## Banner headline',
    body: '{view.headline.body} — from content/pdf-report/takeaway-banners.md / H1 segment / ## Supporting copy',
  },
  grid: TILE_KEYS.map((key, i) => ({
    key,
    label: `{grid[${i}].label} (from content/pdf-report/planning-grid/tile-${String(i+1).padStart(2,'0')}-*.md frontmatter)`,
  ...
  goals: [
    { goal: '{goals[0].goal} — Goal 1 H1 from content/pdf-report/goals/S[n]-*.md',
