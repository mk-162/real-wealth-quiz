...
// Note: state-pension, IHT, and the dual-variant 12th tile were retired in
// the simplification — see types.ts on the rationale. Fixtures still need to
// define the remaining 9 in TileKey order.
function grid(overrides: Partial<Record<import('./types').TileKey, { status: 'green' | 'amber' | 'red' | 'grey'; note: string; label?: string }>>) {
...
const S9: Fixture = {
  ...
  view: {
    ...
    grid: grid({
      ...
      estate:     { status: 'red',   note: '~£1.9m IHT exposure — has will; trust structure not confirmed.' },
